...
@Component({
  selector: 'app-atualizar-dados-cliente',
  templateUrl: './atualizar-dados-cliente.component.html',
  styleUrls: ['./atualizar-dados-cliente.component.css']
})
export class AtualizarDadosClienteComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder
    ...
    , private rota: Router) {
    this.cliente = rotaAtiva.snapshot.params['chave'];
    clienteService.buscarClienteChave(this.cliente).subscribe(result => {
  ...
  editarCliente(): void {
    ...
    this.clienteService.editarClienteChave(this.cliente, editarCliente).subscribe((result) => {
      this._sucesso(result), (error: any) => this._erro(error);
    });
  }
  private _sucesso(resultado: any): void {
    this.rota.navigateByUrl('/acessoCliente');
  }
